...
%Get and plot the linear regression moedl fot the lag which gives the
%best fitting (biggest R^2)
lagofcntr(i)=lags(find(adjR2==max(adjR2)));
 x=[];
 y=[];
        for j=1:n-lagofcntr(i)
             x=[x,cases(j)];
             y=[y,deaths(j+lagofcntr(i))];
        end
        b1=pearson(x,y)*std(y)/std(x);
        bo=mean(y)-b1*mean(x);
        yhat=bo+b1*x;
        ...
        hold on
        plot(x,yhat);
        title(['Linear Regression model for ',char(countries(indexcntr(i))),' ô=',num2str(lagofcntr(i))]);
        ylabel('Deaths/day');
        xlabel(['Cases ',num2str(lagofcntr(i)),' days before']);
        text(max(x),max(y),sprintf('adjR^2=%f\n',adjR2max(i)));
        hold off
        ...
        x=[];
         ...
         for m=1:length(lags)
                 for j=n:-1:(1+abs(lags(m)))
                   x(j,m)=cases(j-lags(k));
                  end
         end
